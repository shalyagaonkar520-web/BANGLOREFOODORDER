      ...
      }),
      {
        ...
        configureServer(server) {
          server.middlewares.use((req, res, next) => {
            ...
            // Handle GET Drinks
            if (req.url && req.url.includes('/api/drinks') && req.method === 'GET') {
              try {
                const drinksPath = path.resolve(__dirname, 'src/data/barDrinks.json');
                if (!fs.existsSync(drinksPath)) {
                  fs.writeFileSync(drinksPath, '[]', 'utf8');
                }
                ...
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: 'Failed to load drinks' }));
              }
              return;
            }

            // Handle POST/PUT Drink (Add/Edit)
            if (req.url && req.url.includes('/api/drinks') && req.method === 'POST') {
              const authHeader = req.headers['authorization'];
              if (!authHeader || !authHeader.includes('mock-jwt-admin-token-123456')) {
                res.writeHead(401, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: 'Unauthorized access' }));
              ...
              req.on('end', () => {
                try {
                  ...
                  let drinks = [];
                  if (fs.existsSync(drinksPath)) {
                    drinks = JSON.parse(fs.readFileSync(drinksPath, 'utf8'));
                  }
                  ...
                  if (drink.id) {
                    ...
                    if (index > -1) {
                      drinks[index] = { ...drinks[index], ...drink };
                    } else {
                      drinks.push(drink);
                    }
                  } else {
                  ...
                  res.writeHead(200, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: true, drink }));
                } catch (e) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, message: 'Invalid drink payload' }));
                }
              });
              return;
            }
            ...
            // Handle DELETE Drink
            if (req.url && req.url.includes('/api/drinks') && req.method === 'DELETE') {
              const authHeader = req.headers['authorization'];
              if (!authHeader || !authHeader.includes('mock-jwt-admin-token-123456')) {
                res.writeHead(401, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: 'Unauthorized access' }));
              ...
              if (!id) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: 'Missing drink ID' }));
                return;
              }
              ...
              try {
                const drinksPath = path.resolve(__dirname, 'src/data/barDrinks.json');
                if (fs.existsSync(drinksPath)) {
                  let drinks = JSON.parse(fs.readFileSync(drinksPath, 'utf8'));
                  drinks = drinks.filter(d => d.id !== id);
                  fs.writeFileSync(drinksPath, JSON.stringify(drinks, null, 2), 'utf8');
                }
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, message: 'Drink deleted successfully' }));
              } catch (e) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: 'Failed to delete drink' }));
              }
              return;
            }
